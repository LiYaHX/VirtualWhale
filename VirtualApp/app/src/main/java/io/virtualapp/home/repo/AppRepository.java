...
public class AppRepository implements AppDataSource {
    ...
    public Promise<List<AppData>, Throwable, Void> getVirtualApps() {
        return VUiKit.defer().when(() -> {
            ...
            for (InstalledAppInfo info : infos) {
                ...
                if (!info.isHook && !VirtualCore.get().isPackageLaunchable(info.packageName)) { // @Hai-Yang Li 2019.10.11
                    continue;
                }
    ...
    private List<AppInfo> convertPackageInfoToAppData(Context context, List<PackageInfo> pkgList, boolean fastOpen) {
        ...
        for (PackageInfo pkg : pkgList) {
            ...
            if (ai.metaData != null) {
//                isHookPlugin = ai.metaData.getBoolean("yahfa.hook.plugin", false);  // 此处需要修改为 whale framework
                isHookPlugin = ai.metaData.getBoolean("whale.hook.plugin", false);
            }   // @ Hai-Yang Li 2019.10.11 end
    ...
    public InstallResult addVirtualApp(AppInfoLite info) {
        ...
        if (info.fastOpen && !info.isHook) {    // @ Hai-Yang Li 2019.10.11 end
            flags |= InstallStrategy.DEPEND_SYSTEM_IF_EXIST;
        }
        if (info.isHook) {   // @ Hai-Yang Li 2019.10.11
