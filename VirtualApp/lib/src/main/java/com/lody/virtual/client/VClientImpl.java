...
import com.lody.whale.WhaleHookMain;
...
public final class VClientImpl extends IVClient.Stub {
    ...
    private void bindApplicationNoCheck(String packageName, String processName, ConditionVariable lock) {
        Log.i(TAG, "bindApplicationNoCheck");
...
//        VirtualCore.get().getComponentDelegate().afterApplicationCreate(mInitialApplication); // @Hai-Yang Li 2019.10.11


        /**--------------------- @2019.10.08 Hai-Yang Li  -------------------*/
        ClassLoader targetClassLoader = mInitialApplication.getClassLoader();
//        WhaleHookMain.hookThirtyProcess(targetClassLoader);
...
//                VLog.w("VirtualHook", "Applying hook "+pluginName);
            ...
            Log.i("VirtualHook", "Applying hook exception");
            e.printStackTrace();
        }
    }
    ...
    private void applyHookPlugin(String apkPath, String libPath, ClassLoader appClassLoader) {
        DexClassLoader dexClassLoader = new DexClassLoader(apkPath,
                ...
                libPath,
                appClassLoader);
        // 解决方案： 是的whale为 lib 的 lib，因此才能被 lib 调用 hook 功能。晚上回家搞这个，否则流量受不了。
//        HookMain.doHookDefault(dexClassLoader, appClassLoader); // HookMain.java 主要实现了 Hook 功能。
//        WhaleHookMain.hookThirtyProcess(appClassLoader);
        WhaleHookMain.doHookDefault(dexClassLoader, appClassLoader);
